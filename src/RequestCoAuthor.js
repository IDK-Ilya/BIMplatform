...
const AddWorkForm = ({ onAdd, onHide }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    const response = await axios.post('https://appmvp.onrender.com/add_scienceWorks', newWork);
    onAdd(response.data);
    setTitle('');
    setDescription('');
    setEmail('');
...
const WorksDisplay = () => {
  ...
  useEffect(() => {
    const fetchWorks = async () => {
      try {
        const response = await axios.get('https://appmvp.onrender.com/get_scienceWorks');
